...
public class Tower {
    ...
    public void pushCup(String type, int size) {
        ...
        Cup newCup = createCup(type, size, color, centerX, yPos);
        newCup.actionPush(items);
        if(!items.contains(newCup)) {
            items.add(newCup);
        }
    ...
    public void popCup() {
        for(int i = items.size() - 1; i >= 0; i--) {
            ...
            if(item.getType().equals("cup")) {
                if(item.hasReachedBase()){
                    showError("No se puede eliminar este tipo de taza.");
                    lastOperationOk = false;
                    return;
                }
    ...
    public void removeCup(int id) {
        for(int n = items.size() - 1; n >= 0; n--) {
            ...
            if(item.getType().equals("cup") && item.getIdentifier() == id) {
                if(item.hasReachedBase()) {
                    showError("No se puede eliminar esta taza de la torre.");
                    lastOperationOk = false;
                    return;
                }
                if(isVisible) item.makeInvisible();
    ...
    public void pushLid(String type, int size) {
        ...
        Lid newLid = createLid(type, size, color, centerX, yPos);
        
        if(!newLid.lidCanEnter(items)) {
            showError("La tapa " + size + " no puede entrar porque su compañera no existe.");
            lastOperationOk = false;
            return;
        }
        
        newLid.actionPush(items);
    ...
    public void popLid() {
        for(int i = items.size() - 1; i >= 0; i--) {
            ...
            if(item.getType().equals("lid")) {
                Lid lid = (Lid) item;
                if(!lid.lidCanExit(items, i)) {
                    showError("La tapa no puede salir, ya que está tapando a su taza compañera.");
                    lastOperationOk = false;
                    return;
                }
                if (isVisible) item.makeInvisible();
    ...
    public void removeLid(int id) {
        for(int n = items.size() - 1; n >= 0; n--) {
            ...
            if(item.getType().equals("lid") && item.getIdentifier() == id) {
                Lid lid = (Lid) item;
                if(!lid.lidCanExit(items, n)) {
                    showError("La tapa no puede salir, ya que está tapando a su taza compañera.");
                    lastOperationOk = false;
                    return;
                }
     ...
     * 
     * @param type El tipo de item que sea: Cup o Lid.
    ...
    /**
     * Método auxiliar privado para instanciar distintos tipos de tazas.
     */
    private Cup createCup(String type, int size, String color, int x, int y) {
        switch(type) {
            case "opener":
                return new OpenerCup(size, color, x, y, cupCounter++);
            
            case "hierarchical":
                return new HierarchicalCup(size, color, x, y, cupCounter++);
            
            case "heavy":
                return new HeavyCup(size, color, x, y, cupCounter++);
            
            default:
                return new NormalCup(size, color, x, y, cupCounter++);
        }
    } // Cierre del método
    
    /**
     * Método auxiliar privado para instanciar diferentes tipos de tapas.
     */
    private Lid createLid(String type, int size, String color, int x, int y) {
        switch(type) {
            case "fearful":
                return new FearfulLid(size, color, x, y, lidCounter++);
            
            case "crazy":
                return new CrazyLid(size, color, x, y, lidCounter++);
            
            default:
                return new NormalLid(size, color, x, y, lidCounter++);
        }
    } // Cierre del método
} // Cierre de la clase